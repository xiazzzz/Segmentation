...
class HybridAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.stack([x3, x2], -2)  # 2,64,64,3,128             2,64,64,2,128
        x = torch.mul(x, logits.unsqueeze(-1))  # 后面2,64,64,3,1
        x = torch.sum(x, -2)
        x = torch.concat([x, x3], dim=-1)  # 2,64,64,256
        # B = x.permute(0, 3, 1, 2).shape[1]
        # x = nn.Conv2d(B, 128, kernel_size=3, padding=1)
