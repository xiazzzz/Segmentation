...
class HybridAttention(nn.Module):
    def __init__(self, dim, heads=8, dropout=0.0, depth=0, wsize=-1, psize=-1, cnn_expansion=4):
        ...
        self.cnn5 = BottleneckV1(dim, dim, expansion=cnn_expansion)
    ...
    def forward(self, x):
        ...
        x4 = self.cnn5(x.permute(0, 3, 1, 2))
...
class NomMerAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.merge1(x)
        x1 = x             #2,32,32,96
        x1 = self.PH1(x1.permute(0, 3, 1, 2) )

        x = self.transformer2(x)
        x = self.merge2(x)
        x2 = x            #2,16,16,192
        x2 = self.PH2(x2.permute(0, 3, 1, 2) )
        ...
        x = self.merge3(x)
        B = x.shape[0]
        x3 = x.contiguous().view(B, 8, 8, 1024)   #2,8,8,384
        x3 = self.PH3(x3.permute(0, 3, 1, 2) )
