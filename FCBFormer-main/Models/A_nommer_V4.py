...
class C2f(nn.Module):
    def __init__(self, in_channels, out_channels, n=3, shortcut=False):
        super(C2f, self).__init__()
        c_out = 0.5
        self.n = n
        self.shortcut = shortcut
        self.Conv1 = nn.Sequential(
            nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=1, stride=1),  # 256, 256
            nn.BatchNorm2d(num_features=out_channels),
            nn.SiLU()
        )

        self.Split = nn.Sequential(
            nn.Conv2d(in_channels=out_channels, out_channels=int(c_out * out_channels), kernel_size=1, stride=1),
            # 256, 128
            nn.BatchNorm2d(num_features=int(c_out * out_channels)),
            nn.SiLU()
        )
        ...
        for i in range(self.n):
            layer = nn.Sequential(
                nn.Conv2d(in_channels=int(c_out * out_channels), out_channels=int(c_out * out_channels), kernel_size=1,
                          stride=1),  # 128, 128
                nn.BatchNorm2d(num_features=int(c_out * out_channels)),
                nn.SiLU(),

                nn.Conv2d(in_channels=int(c_out * out_channels), out_channels=int(c_out * out_channels), kernel_size=3,
                          stride=1,  # 128, 128
                          padding=1),
                nn.BatchNorm2d(num_features=int(c_out * out_channels)),
                nn.SiLU()
            )
            self.Bottleneck.append(layer)

        self.Conv2 = nn.Sequential(
            nn.Conv2d(in_channels=int(0.5 * (self.n + 2) * out_channels), out_channels=out_channels, kernel_size=1,
                      stride=1),  # 384, 256
            nn.BatchNorm2d(num_features=out_channels),
            nn.SiLU()
        )
    ...
    def forward(self, x):
        ...
        x1 = self.Split(x)  # 256, 128
        lis_cat = []
        for Bottleneck in self.Bottleneck:
            x2 = Bottleneck(x1)  # 128, 128
            # if self.shortcut is True:
            x2 = x1 + x2
            lis_cat.append(x2)
        x3 = torch.cat(lis_cat, dim=1)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, expansion=2, stride=1, groups=1, base_width=64, dilation=1, norm_layer=None):
...
class NomMerAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.merge1(x)
        x1 = x.permute(0, 3, 1, 2)  # 2,32,32,96

        x = self.transformer2(x)
        x = self.merge2(x)
        x2 = x.permute(0, 3, 1, 2)  # 2,16,16,192
        ...
        x = self.merge3(x)
        B = x.shape[0]
        x3 = x.contiguous().view(B, 8, 8, 768)  # 2,8,8,384
        x3 = self.conv(x3.permute(0, 3, 1, 2))
        x3 = self.PH4(x3)

        cls_tokens = repeat(self.cls_token, '() n d -> b n d', b=b)
        # x = torch.cat((cls_tokens, x), dim=1)
        x = self.transformer4(x)  # 2,64,768
        B = x.shape[0]
        x = x.contiguous().view(B, 8, 8, 768)
        x = self.conv(x.permute(0, 3, 1, 2))  # 2,768,8,8
        x4 = x
        x4 = self.PH4(x4)
...
if __name__ == '__main__':
    ...
    # model = NomMerAttn(input_size=256).to(device)
    # img = torch.rand([2,3,256,256]).to(device)
    # result = model(img)
    # print(result.shape)  #2,1,256,256
    #
    # img1 = torch.rand([2, 30, 256, 256]).to(device)
    # C2f1 = C2f(30,10).to(device)
    # img1 = C2f1(img1)
    # print(img1.shape)
