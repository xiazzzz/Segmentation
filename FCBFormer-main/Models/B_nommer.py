...
class HybridAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x3 = self.SE_Block(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
        # hybrid_x = x1 + x2 + x3
        hybrid_x = (x1 + x2) * x3 + x
...
class NomMerAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.transformer1(x)          #192
        x = self.merge1(x)                #32  192

        x = self.transformer2(x)
        x = self.merge2(x)             #16   384

        x = x + self.pos_embedding
        x = torch.flatten(x, 1, 2)
        x = self.transformer3(x)
        x = self.merge3(x)                 #8,768
        ...
        x = x.contiguous().view(B, 256, 256, 1)
        x = x.permute(0, 3, 1, 2)
        # x = x[:, 0]
        # out = self.mlp_head(x)
        return x
