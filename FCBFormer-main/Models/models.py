...
from Models.B_nommer import NomMerAttn
...
class TB2(nn.Module):
    def __init__(self,img_size=256):
        ...
        #     RB(256, 32),nn.Conv2d(32, 1, kernel_size=1),
        # )
...
class TB4(nn.Module):
    ...
    def forward(self,x):
        x = self.swinv2(x)[0].permute(0, 3, 1, 2)
        x = self.up_tosize(x)
        return x
...
class FCBFormer(nn.Module):
    def __init__(self, size=256):
        ...
        self.TB4 = TB4(img_size=256)
        # self.TB3 = TB3(img_size=256)
        self.FCB = FCB(in_resolution=size)
        self.PH = nn.Sequential(
            RB(768, 32), nn.Conv2d(32, 1, kernel_size=1)
        )
